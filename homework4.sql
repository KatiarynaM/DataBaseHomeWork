...
select auto.color, count(auto.REGNUM) as 'Количество' from auto
...
SELECT auto.mark, count(auto.REGNUM) as 'Количество', (select count(auto.REGNUM) from auto) - count(auto.REGNUM)  as 'Количество прочих'
FROM  auto
group by auto.mark;


/*Вывести марку авто - а так же сколько авто такой марки в таблице , вывести записи где  
 количество авто такой марки больше 2, записи не должны дублироваться*/
 
SELECT auto.mark, count(auto.REGNUM) as 'Количество'
FROM  auto
group by auto.mark
having count(auto.REGNUM) >2;
...
CREATE TABLE man_big_city SELECT PHONENUM, FIRSTNAME, LASTNAME
from  man
join city 
on man.CITYCODE = city.CITYCODE WHERE city.PEOPLES >1000000;

SELECT man_big_city.FIRSTNAME, man_big_city.LASTNAME, auto.mark, auto.color
from man_big_city
join auto
on  man_big_city.PHONENUM = auto.PHONENUM
;
